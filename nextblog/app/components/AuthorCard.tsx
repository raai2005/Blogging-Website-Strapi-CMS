...
export default function AuthorCard({ author, compact = false }: AuthorCardProps) {
  ...
  const username = attrs.username || author.username || 'Anonymous';
  const bio = attrs.bio || author.bio;
  const avatar = attrs.avatar?.data || author.avatar?.data;
  const avatarUrl = avatar?.attributes?.url || avatar?.url;
  const twitter = attrs.twitter || author.twitter;
  const github = attrs.github || author.github;
  const linkedin = attrs.linkedin || author.linkedin;
